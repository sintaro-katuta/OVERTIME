...
func _init() -> void:
	...
	legacy_save.erase("has_selected_weapon")
	var migrated = ProgressionStateData.new()
	assert(migrated.load_save_data(legacy_save) == OK)
	assert(migrated.preparation_view().selected_weapon_id == "kestrel_762")
	...
	assert(restored.preparation_view().selected_weapon_id == "kestrel_762")
